...
import com.aca.dbflow.KonveRate;
...
//        TPLBind.bind(this, spnTpl, kodeProduk, vehicle_category);
...
//		Utility.BindPAKonve(spnPA, FillKonvensionalActivity.this, FillKonvensionalActivity.this);
    ...
    private void LoadDB() {
        ...
        /*
            ...
            TPLBind.select(spnTpl, String.valueOf(c.getDouble(21)));
            PABind.select(spnPA, c.getString(22));
...
//				maxTahun = 7;
    ...
    private AlertDialog showConfirmDelete(
            final Context ctx, final long rowId,
...
//		}
    ...
    private double getPremiPerluasan() throws ParseException {
        try {
            ...
            String jaminan = "";
            String wilayah = "0";
            String vehicleCategory = "0";

            String tanggal = jangkaWaktuEff.getText().toString();
            Date effDate = tanggal.isEmpty() ? UtilDate.getDate().toDate() : UtilDate.toDate(tanggal, UtilDate.STANDARD_DATE).toDate();
...
//            PA = nf.parse(((SpinnerGenericItem)spnPA.getSelectedItem()).getCode()). doubleValue();
            PA = Double.parseDouble(PABind.getID(spnPA));

//		double rateTJH = getRatePerluasan(jaminan, wilayah, vehicle_category, "5", String.valueOf(Tjh));
            double ratePAP = getRatePerluasan(jaminan, wilayah, vehicleCategory, "6", "0");
            double ratePAD = getRatePerluasan(jaminan, wilayah, vehicleCategory, "7", "0");

//		premiTJH = Tjh *  rateTJH / 100 ;
            premiTJH = PerluasanPremi.getPremi(Double.parseDouble(TPLBind.getID(spnTpl)), Var.TPL, effDate, kodeProduk, vehicle_category);
    ...
    private double getRate() throws InterruptedException, ParseException {
        ...
        try {
            ...
            String mBeginDate = jangkaWaktuEff.getText().toString();
            if (mBeginDate.isEmpty())
                return 0;

            Date beginDate = UtilDate.toDate(mBeginDate, UtilDate.STANDARD_DATE).toDate();
            KonveRate konveRate;

            if (isTruk) {
                konveRate = KonveRate.getRate(Wilayah, vehicle_category, exco, jaminan, 0, beginDate);
            } else {
                konveRate = KonveRate.getRate(Wilayah, vehicle_category, exco, jaminan, v_tsi, beginDate);
            }
/*
            if (isTruk)
...
*/
            ...
            if (konveRate != null) {
                ...
                if (rbRateAtas.isChecked()) {
                    v = konveRate.Rate_Atas;
                } else if (rbRateTengah.isChecked()) {
                    v = konveRate.Rate_Tengah;
                } else if (rbRateBawah.isChecked()) {
                    v = konveRate.Rate_Bawah;
                }
                Log.d("FillKonvensionalActivity getRate rate basic ", v + "" );
		...
		/*	if (c.moveToFirst()) {
            ...
            Log.i(TAG, "::getRate:" + "rate total : " + v);
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            if (c != null)
    ...
    private double getRatePerluasan( String jaminan, String wilayah, String vehicle_category, String exco, String jnp ) {
        try {
            ...
            if (isTruk)
                ...
                konveRate = KonveRate.getRate(wilayah, vehicle_category, exco, jaminan, Double.parseDouble(jnp), beginDate);

            if (konveRate == null)
                return 0;


            return konveRate.Rate_Bawah;
